...
- **メソッド**
  - `__init__(screen, clock, font, title_font)`: リソースを受け取り、画像読込とボタン設定を行う。
  ...
  - `_draw_layout(mouse_pos, mouse_down) -> None`: タイトル、表示パネル、画像（存在時は枚数インジケータ付き）を描画。前/次ボタンのクリックで `current_index` を循環変更。
